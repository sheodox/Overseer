...
type NotificationTypeName = keyof Pick<UserSettings,
...
const pushReady = prisma.appMetadata.findFirst({
...
export async function createNotificationForUser(userId: string, data: NotificationCreateData, notificationTypeName: NotificationTypeName) {
    ...
    await pushReady;
    await Promise.all(userSubscriptions.map(async userPushSub => {
        try {
