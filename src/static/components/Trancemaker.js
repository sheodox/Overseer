...
class Trancemaker {
	constructor() {
		...
		const backgroundColor = 0x151d29;
		this.renderer.setClearColor(backgroundColor);
		document.body.appendChild(this.renderer.domElement);
		this.renderer.domElement.classList.add('trancemaker');
	...
	createLowPolyGeometry() {
		...
		for (let i = 0; i + 1 < max; i++) {
			for (let j = 0; j + 1 < max; j++) {
				geo.faces.push(
					new THREE.Face3(getVertexNumber(i + 1, j), getVertexNumber(i, j + 1), getVertexNumber(i, j)),
					new THREE.Face3(getVertexNumber(i + 1, j), getVertexNumber(i + 1, j + 1), getVertexNumber(i, j + 1))
				);
			}
		}
